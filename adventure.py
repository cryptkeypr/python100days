print('''
*******************************************************************************
          |                   |                  |                     |
 _________|________________.=""_;=.______________|_____________________|_______
|                   |  ,-"_,=""     `"=.|                  |
|___________________|__"=._o`"-._        `"=.______________|___________________
          |                `"=._o`"=._      _`"=._                     |
 _________|_____________________:=._o "=._."_.-="'"=.__________________|_______
|                   |    __.--" , ; `"=._o." ,-"""-._ ".   |
|___________________|_._"  ,. .` ` `` ,  `"-._"-._   ". '__|___________________
          |           |o`"=._` , "` `; .". ,  "-._"-._; ;              |
 _________|___________| ;`-.o`"=._; ." ` '`."\` . "-._ /_______________|_______
|                   | |o;    `"-.o`"=._``  '` " ,__.--o;   |
|___________________|_| ;     (#) `-.o `"=.`_.--"_o.-; ;___|___________________
____/______/______/___|o;._    "      `".o|o_.--"    ;o;____/______/______/____
/______/______/______/_"=._o--._        ; | ;        ; ;/______/______/______/_
____/______/______/______/__"=._o--._   ;o|o;     _._;o;____/______/______/____
/______/______/______/______/____"=._o._; | ;_.--"o.--"_/______/______/______/_
____/______/______/______/______/_____"=.o|o_.--""___/______/______/______/____
/______/______/______/______/______/______/______/______/______/______/_____ /
*******************************************************************************
''')
print("Welcome to Treasure Island.")
print("Your mission is to find the treasure.") 

answer = input('> You\'re at a cross road. Where do you want to go? Type "left" or "right".\n').lower()

if answer == "left" or answer == "l":
  answer1 = input('\n> You came to a lake. There is an island in the middle of the lake. Type "wait" to wait for a boat. Type "swim" to swim across.\n')
  if answer1 == "wait" or answer1 == "w":
    answer2 = input("\n> You arrive at the island unharmed. There is a house with 3 doors. One red, one yellow, and one blue. Which color do you choose?\n")
    if answer2 == "yellow" or answer2 == "y":
      print(''' 
                                                   ,%%%%%%%%,
                                                  %%/\%%%%/\%%,
                                         ,,,,,   %%%\C "" J/%%%,
                        ,%%%%%%%,        |||||   %%%/ 6  6 \%%%%       .%
                      ,%%/\%%%%/\%,    @@@@@@@@@ %%%|  _    %%%%     .%%`
                     ,%%%\c "" J/%%,   {~*~*~*~} '%%(__Y__)%%%%'     %%`
%.                   %%%%/ d  b \%%% @@@@@@@@@@@@@'%%%\-/`%%%%;       \\
`%%.         __      %%%%    _  |%%% {~* WINNER*~}  '%%%%%%%`  \       ))
 `%%      .-'  `"~--"`%%%%(=_Y_=)%%' {   WINNER  }   |          '.    //
  //    .'     `.     `%%%%`\7/%%%'__{~~*~~*~~*~~}   | |  /       \  //
 ((    /         ;      `%%%%%%%'____@@@@@@@@@@@@@   | | (         \//
 `.`--'         ,'   _,`-._____`-,      __) (__    __| | _\         /
   `"""`._____  `--,`          `)))    /_______\  (((((((___________)
              `~"-)))
      ''')
      print("\n> ![WINNER WINNER CHICKEN DINNER]!")
    elif answer2 == "blue" or answer2 == "b":
      print('''

                       ,      ,
            /(.-""-.)\
        |\  \/      \/  /|
        | \ / =.  .= \ / |
        \( \   o\/o   / )/
         \_, '-/  \-' ,_/
           /   \__/   \
           \ \__/\__/ /
         ___\ \|--|/ /___
       /`    \      /    `\
  jgs /       '----'       \
      ''')
      print("\n> [GAME OVER]: You were eaten by beasts!")
    elif answer2 == "red" or answer2 == "r":
      print('''
               (  .      )
           )           (              )
                 .  '   .   '  .  '  .
        (    , )       (.   )  (   ',    )
         .' ) ( . )    ,  ( ,     )   ( .
      ). , ( .   (  ) ( , ')  .' (  ,    )
     (_,) . ), ) _) _,')  (, ) '. )  ,. (' )
 jgs^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^''')
      print("\n> [GAME OVER]: You were burned by fire!")
  else:
    print('''
                 |
                 |
                ,|.
               ,\|/.
             ,' .V. `.
            / .     . \
           /_`       '_\
          ,' .:     ;, `.
          |@)|  . .  |(@|
     ,-._ `._';  .  :`_,' _,-.
    '--  `-\ /,-===-.\ /-'  --`
   (----  _|  ||___||  |_  ----)
    `._,-'  \  `-.-'  /  `-._,'
             `-.___,-' 

    ''')
    print("\n> [GAME OVER]: You were attacked by a trout!")
else:
  print('''
                                        +---+
                                      |\   \
  +-----------------------------+     | +---+
   \                      +-----------+ |   |
    \                      \            |   |
     \                 |/   +-----------+   |
      \                (c_      |   \ | |   |
       \                \       |    \| |   |
        \                       |     | |   |
         \                      |     + |   |
          \                     |      \| DM|
           \--------------------+       +---+
            \                    \        \
             \                    \        \
              +-----------------------------+
  ''')
  print("\n> [GAME OVER]: You fell in a hole!")
